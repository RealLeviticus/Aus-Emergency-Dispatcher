...
export function ScenePacksDialog({
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/40" onMouseDown={close}>
      <div
        className="win-window flex max-h-[88vh] w-[620px] flex-col"
        onMouseDown={(e) => e.stopPropagation()}
        role="dialog"
        aria-label="Scene object packs"
      >
        <div className="win-titlebar flex items-center justify-between px-2 py-[2px]">
          <span className="font-bold">{firstRun ? 'Set up scene objects before you fly' : 'Scene object packs'}</span>
          <button type="button" className="win-titlebar-btn" onClick={close} aria-label="Close">
            ×
          </button>
        </div>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-3 text-[12px] leading-snug">
          {firstRun && (
            <div className="win-sunken mb-2 border-l-4 border-[#a05000] px-2 py-[6px] text-[11px]">
              <b>Some scene object packs aren&rsquo;t installed yet.</b> Without them the dispatcher falls back to
              basic shapes at MVA, rescue and fire jobs. Set them up now, then fully restart MSFS — you only do
              this once.
            </div>
          )}
          <p className="mb-2">
            The dispatcher drops real vehicles, wrecks, crew, patients, fire and smoke at each job. Those props
            come from a few free <b>flightsim.to</b> packs. Their licences don&rsquo;t let this app bundle or
            download them, so it&rsquo;s a one-time manual step:
          </p>
          <ol className="mb-3 ml-5 list-decimal space-y-[2px]">
            <li>
              Open the packs folder and read the note there, then download each ZIP from flightsim.to (sign in
              first).
            </li>
            <li>
          ...
          <div className="mb-2 flex flex-wrap gap-2">
            <button type="button" className="win-btn px-3 py-[2px]" onClick={() => void sim.addonOpenFolder()}>
              Open packs folder
            </button>
            <button
              type="button"
              className="win-btn is-default px-3 py-[2px]"
              disabled={busy}
              onClick={() => void install()}
            >
              {busy ? 'Installing…' : `Install downloaded packs${zipCount ? ` (${zipCount} ZIP${zipCount > 1 ? 's' : ''})` : ''}`}
            </button>
            <button type="button" className="win-btn px-3 py-[2px]" disabled={busy} onClick={() => void refresh()}>
              Rescan
            </button>
          </div>

          <div className="win-sunken mb-2 px-2 py-1 text-[11px]">
            {communities.length === 0 ? (
              <span className="text-[#a00000]">
                No MSFS Community folder detected — start MSFS once, then Rescan.
              </span>
            ) : (
              <span>
              ...
              {(status?.packs ?? []).map((p) => (
                <div key={p.id} className="win-sunken px-2 py-[6px]">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <span
                      className={
                        p.installed ? 'text-[#006000]' : p.partial ? 'text-[#a05000]' : 'text-[#a00000]'
                      }
                    >
                  ...
                  <div className="mt-1 flex items-center gap-2 text-[11px]">
                    <button
                      type="button"
                      className="win-btn px-2 py-0"
                      onClick={() => void sim.addonOpenUrl(p.url)}
                    >
                      Open page
                    </button>
          ...
          <p className="mt-2 text-[10px] text-[#606060]">
            Packs stay under their authors&rsquo; own licences and copyright. This app never re-hosts or
            redistributes them; it only extracts the copies you download.
          </p>
        </div>
        ...
        <div className="flex items-center justify-between gap-2 border-t border-[#808080] p-2">
          ...
          <button type="button" className="win-btn px-4 py-[2px]" onClick={close}>
